...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'endoflife.date');
}
...
async function listProducts(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/all.json`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
...
async function getProduct(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/${encodeURIComponent(product)}.json`, {
...
async function getCycle(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/${encodeURIComponent(product)}/${encodeURIComponent(cycle)}.json`, {
    headers: { Accept: 'application/json', 'User-Agent': UA },
  });
  if (res.status === 404)
